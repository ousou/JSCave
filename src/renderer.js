...
(function exposeRenderer(root, factory) { const api = factory(); root.JavaCaveRenderer = api; if (typeof module !== 'undefined' && module.exports) module.exports = api; })(globalThis, function () {
  ...
  class Renderer {
    ...
    game(game) { const c = this.ctx; for (let i = 0; i < 32; i += 1) { const shade = Math.abs(((game.gameCount + i) % 16) - 8) * 16; this.color(`rgb(${128 - shade},255,${128 - shade})`); c.fillRect(i * 4, 0, 4, 128); this.color(`rgb(${shade},0,0)`); c.fillRect(i * 4, game.map[0][i], 4, game.map[1][i] - game.map[0][i]); if (game.map[2][i] !== -1) { this.color('rgb(0,255,128)'); c.fillRect(i * 4, game.map[2][i], 4, 16); } } this.color('rgb(128,128,255)'); c.fillRect(0, 128, 128, 32); this.text('bold 16px Times New Roman', '#fff', `Score : ${game.score}`, 20, 150); this.color('rgb(128,128,255)'); for (const offset of [-1, 0, 1]) this.pixelLine(30, game.trailStartY + offset, 34, game.y + offset); }
